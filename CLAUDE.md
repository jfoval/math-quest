...
- Star Base is a 24×24 plot (`N` in base.js) with terrain `ZONES` (Frost Field, Crater Flats). Shop is grouped: buyable / `planet:` gated / `champion:` gated ('any' or an op) / `season:` (month numbers) / `gift: true` earned swag. Earned swag rules live in app.js `GIFT_RULES` (one predicate per item key; `how` text on the item) and are granted inside `checkBadges`, so they show on any summary as a badge-style card. Counters they read: `best.streak/lightning/bosses/perfects/raceWins/games[kind]`, `planDays`, `opMissions.mix`, level, champions. `aura` item = orbiting rings around the avatar (`auraSVG`, SMIL-animated), granted with all four champions.
